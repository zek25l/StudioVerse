...
app.delete('/api/submissions/:id', requireAdmin, async (request, response) => {
  const submissions = await readSubmissions();
  const remaining = submissions.filter(submission => submission.id !== request.params.id);
  if (remaining.length === submissions.length) return response.status(404).json({ error: 'Submission not found.' });
