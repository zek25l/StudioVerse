...
function Write-Users($users) {
    ...
    $items = @($users | Where-Object { $_ -and $_.username })
    if ($items.Count -eq 0) { '[]' | Set-Content -Path $temporary -Encoding UTF8 } else { $items | ConvertTo-Json -Depth 5 | Set-Content -Path $temporary -Encoding UTF8 }
...
function Write-Submissions($submissions) {
    ...
    $items = @($submissions | Where-Object { $_ -and $_.id })
    if ($items.Count -eq 0) { '[]' | Set-Content -Path $temporary -Encoding UTF8 } else { $items | ConvertTo-Json -Depth 8 | Set-Content -Path $temporary -Encoding UTF8 }
...
function Test-Password($password, $user) {
    ...
    $candidateBytes = [Convert]::FromBase64String($candidate.hash)
    $storedBytes = [Convert]::FromBase64String($user.passwordHash)
    if ($candidateBytes.Length -ne $storedBytes.Length) { return $false }
    $difference = 0
    for ($index = 0; $index -lt $candidateBytes.Length; $index++) { $difference = $difference -bor ($candidateBytes[$index] -bxor $storedBytes[$index]) }
...
try {
    while ($listener.IsListening) {
        ...
        try {
            ...
            if ($context.Request.HttpMethod -eq 'DELETE' -and $path -like '/api/submissions/*') {
                if (-not (Test-Admin $context)) { Send-Json $context 401 @{ error = 'Admin access required.' }; continue }
                $id = $path.Substring('/api/submissions/'.Length); $submissions = @(Read-Submissions); $remaining = @($submissions | Where-Object { $_.id -ne $id })
                if ($remaining.Count -eq $submissions.Count) { Send-Json $context 404 @{ error = 'Submission not found.' }; continue }
                Write-Submissions $remaining; $context.Response.StatusCode = 204; $context.Response.Close(); continue
            }
