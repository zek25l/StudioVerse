...
class OrbitHandler(BaseHTTPRequestHandler):
    ...
    def do_PATCH(self):
        ...
        if not route.startswith('/api/submissions/') or not self.is_admin():
            message = 'Admin access required.' if route.startswith('/api/submissions/') else 'Not found'
            return self.send_json(401 if route.startswith('/api/submissions/') else 404, {'error': message})
        try:
